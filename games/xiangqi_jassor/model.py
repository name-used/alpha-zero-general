...
class Model:
    def __init__(self, batch_size):
        self.net = CNNNet(in_dim=18, hid_dim=256, out_dim=32400, num_blocks=10)
    ...
    def predict(self, state):
        self.net.eval()
        self.net.cuda()
        board = self.board2cnn(state)
        board = torch.FloatTensor(board.astype(np.float32)).unsqueeze(0).cuda()
        with torch.no_grad():
            pi, v = self.net(board)
        return F.softmax(pi, dim=1).cpu().numpy()[0], v.item()

    def board2cnn(self, state: GameState):
        # 盘面棋子表示：[h, w] -7 ~ 7 -> [h, w, 15]
        board = np.eye(15)[state.board + 7]
        # 附加层：当前走子表示
        on_turn = state.board * state.player_turn > 0
        # 附加层：剩余军粮表示
        ration = np.sign(state.board)
        ration[ration > 0] = state.red_rations / 100
        ration[ration < 0] = state.black_rations / 100
        # 附加层：红棋停招剩余次数表示
        stop = state.board == 1
        cnn_board = np.concatenate([
            board.astype(np.float32),
            on_turn[..., None].astype(np.float32),
            ration[..., None].astype(np.float32),
            stop[..., None].astype(np.float32),
        ], axis=2)
        # [18, 10, 9]
        return cnn_board.transpose((2, 0, 1))
